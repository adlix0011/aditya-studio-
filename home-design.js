...
(()=>{
 ...
 const nav=()=>[['home','⌂','होम'],['orders','♧','मेरे ऑर्डर'],['post','＋','पोस्ट करें'],['receive','🛵','Delivery Receive'],['messages','▤','बातचीत']].map(([id,icon,label])=>`<button data-go="${id}" class="${tab===id?'active':''}"><span>${icon}</span>${label}</button>`).join('');
 ...
 const receive=()=>{const posts=(state.orders||[]).filter(o=>o.kind!=='delivery-service'&&['open','chat'].includes(o.status));return `<section class="delivery-receive-page"><div class="receive-hero"><div class="bike-animation" aria-hidden="true"><span class="bike-rider">🧑‍✈️</span><span class="bike">🛵</span><i></i><i></i><i></i></div><div><p class="eyebrow">DELIVERY RECEIVE</p><h1>नई delivery requests</h1><p>अपने आसपास की खुली मांग चुनें और delivery शुरू करें।</p></div></div><div class="market-list">${posts.map(o=>`<article class="market-card"><h3>${esc(o.title||'Delivery request')}</h3><p class="muted">${esc(o.area||'Pickup')} → ${esc(o.deliveryVillage||o.address||'Delivery location')}</p><button class="btn accept-wide" data-go="orders">देखें और स्वीकार करें →</button></article>`).join('')||'<div class="empty">अभी कोई delivery request उपलब्ध नहीं है।</div>'}</div></section>`};
