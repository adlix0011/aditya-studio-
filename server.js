...
const DELIVERY_CONFIRM_TIMEOUT_MS = 5 * 60 * 1000;
...
function validTotp(secret, code) { const clean=String(code||'').replace(/\D/g,''); if(!/^[0-9]{6}$/.test(clean)||!secret)return false; const key=base32Decode(secret),slot=Math.floor(Date.now()/30000); for(let drift=-1;drift<=1;drift++){const b=Buffer.alloc(8),counter=slot+drift;b.writeUInt32BE(Math.floor(counter/0x100000000),0);b.writeUInt32BE(counter>>>0,4);const h=crypto.createHmac('sha1',key).update(b).digest(),offset=h[h.length-1]&15,n=((h[offset]&127)<<24)|(h[offset+1]<<16)|(h[offset+2]<<8)|h[offset+3];if(String(n%1000000).padStart(6,'0')===clean)return true;} return false; }
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === 'GET' && urlPath === '/api/settings') {
    // Keep settings short-cached: image bytes remain cached in R2, while admin
    // crop/text changes become visible to customers quickly after refresh.
...
function computeOrderFees(subtotal, settingsFees) {
    ...
    try {
      ...
      if (!otpMatches(row, otp)) {
        if (row.attempts >= PIN_RESET_OTP_MAX_ATTEMPTS) {
          row.lockedAt = new Date().toISOString(); row.lockedUntil = now + PIN_RESET_OTP_LOCK_MS;
          acc.pinResetRequested = false; acc.pinResetRequestedAt = null;
          saveOtpRequests(list); saveAccounts(accounts); recordAuthFailure(req, mobile + ':pin-reset-verify');
          return sendJSON(res, 429, { ok: false, error: 'otp-locked', message: '3 गलत OTP attempts हो गए। PIN reset 15 मिनट के लिए block है। इसके बाद नया OTP request करें।', lockedUntil: row.lockedUntil });
        }
        saveOtpRequests(list); recordAuthFailure(req, mobile + ':pin-reset-verify');
  ...
  if (req.method === 'POST' && urlPath === '/api/local-delivery/orders') {
    try {
      ...
      // A chat, quote and confirmation belong to one delivery candidate.  Older
      // orders are migrated lazily so existing orders do not lose their chat.
      const candidateSessions=o=>{
        o.deliveryCandidates=Array.isArray(o.deliveryCandidates)?o.deliveryCandidates:[];
        if(o.providerMobile&&!o.deliveryCandidates.some(c=>sameMobile(c.mobile,o.providerMobile)))o.deliveryCandidates.unshift({mobile:o.providerMobile,name:o.providerName||'Delivery helper',acceptedAt:o.acceptedAt||o.createdAt});
        o.candidateSessions=Array.isArray(o.candidateSessions)?o.candidateSessions:[];
        for(const c of o.deliveryCandidates){
          if(!o.candidateSessions.some(s=>sameMobile(s.mobile,c.mobile))){
            const legacy=sameMobile(c.mobile,o.providerMobile);
            o.candidateSessions.push({mobile:c.mobile,name:c.name||'Delivery helper',acceptedAt:c.acceptedAt||o.acceptedAt||o.createdAt,messages:legacy?(Array.isArray(o.messages)?o.messages:[]):[],paymentRequest:legacy?o.paymentRequest:null,deliveryConfirmRequest:legacy?o.deliveryConfirmRequest:null});
          }
        }
        return o.candidateSessions;
      };
      const sessionFor=(o,mobile)=>candidateSessions(o).find(s=>sameMobile(s.mobile,mobile));
      const viewFor=(o,mobile)=>{
        const copy=JSON.parse(JSON.stringify(o));
        candidateSessions(copy);
        if(sameMobile(copy.ownerMobile,mobile))return copy;
        const s=sessionFor(copy,mobile);
        if(s){copy.providerMobile=s.mobile;copy.providerName=s.name;copy.messages=Array.isArray(s.messages)?s.messages:[];copy.paymentRequest=s.paymentRequest||null;copy.deliveryConfirmRequest=s.deliveryConfirmRequest||null;}
        return copy;
      };
      ...
      for(const listedOrder of orders){
        candidateSessions(listedOrder);
        if(listedOrder.status!=='chat')continue;
        const settled=(listedOrder.candidateSessions||[]).find(c=>c.paymentRequest?.status==='applied');
        if(!settled)continue;
        listedOrder.status='booked';listedOrder.providerMobile=settled.mobile;listedOrder.providerName=settled.name;listedOrder.deliveryCandidates=[{mobile:settled.mobile,name:settled.name,acceptedAt:settled.acceptedAt}];listedOrder.candidateSessions=[settled];listedOrder.messages=Array.isArray(settled.messages)?settled.messages:[];listedOrder.paymentRequest=settled.paymentRequest;listedOrder.deliveryConfirmRequest=settled.deliveryConfirmRequest||null;promotedSettledOrder=true;
      }
      let autoRejectedOrder=false,autoRejectLocksDirty=false,autoRejectLocks=[];try{autoRejectLocks=JSON.parse(fs.readFileSync(LOCAL_DELIVERY_LOCKS_FILE,'utf8'))||[]}catch(_){}
      for(const listedOrder of orders){
        if(!['booked','delivering'].includes(listedOrder.status))continue;
        const requestedAt=new Date(listedOrder.neededBy||'').getTime(),acceptedAt=new Date(listedOrder.acceptedAt||listedOrder.createdAt||0).getTime(),deadline=Number.isFinite(requestedAt)&&requestedAt>0?requestedAt:(Number.isFinite(acceptedAt)&&acceptedAt>0?acceptedAt+24*60*60*1000:0);
        if(!deadline||deadline>Date.now())continue;
        listedOrder.status='rejected';listedOrder.rejectedAt=new Date().toISOString();listedOrder.updatedAt=listedOrder.rejectedAt;listedOrder.messages=Array.isArray(listedOrder.messages)?listedOrder.messages:[];listedOrder.messages.push({sender:'system',text:'⌛ Delivery समय पर पूरी नहीं हुई, इसलिए order अपने-आप Incomplete कर दी गई।',time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})});
        const lock=autoRejectLocks.find(x=>String(x.orderId)===String(listedOrder.id)&&sameMobile(x.mobile,listedOrder.ownerMobile)&&x.status==='locked'),ownerAccount=accounts.find(a=>sameMobile(a.mobile,listedOrder.ownerMobile));
        if(lock&&ownerAccount){walletTxn(ownerAccount,'credit',Number(lock.amount||0),{reason:'Local Delivery incomplete refund · '+listedOrder.id,source:'local_delivery_auto_reject_refund',orderId:listedOrder.id,operationKey:'local-auto-reject-refund:'+listedOrder.id});lock.status='refunded';lock.refundedAt=listedOrder.rejectedAt;autoRejectLocksDirty=true;}
        addNotification({title:'⌛ Order incomplete',body:'“'+String(listedOrder.title).slice(0,90)+'” समय पर delivery नहीं हुई, इसलिए History में भेज दिया गया।',mobile:listedOrder.ownerMobile,kind:'local-delivery-incomplete',orderId:listedOrder.id});autoRejectedOrder=true;
      }
      if(promotedSettledOrder||autoRejectedOrder){writeOrders();if(autoRejectLocksDirty)fs.writeFileSync(LOCAL_DELIVERY_LOCKS_FILE,JSON.stringify(autoRejectLocks.slice(0,1000),null,2));saveAccounts(accounts);}
      if(body.action==='list'){const expired=o=>{const due=new Date(o.neededBy||'');return !Number.isNaN(due.getTime())&&due.getTime()<=Date.now()};return sendJSON(res,200,{ok:true,orders:orders.filter(o=>(o.kind==='delivery-service'&&o.serviceActive!==false)||sameMobile(o.ownerMobile,acc.mobile)||sameMobile(o.providerMobile,acc.mobile)||(o.deliveryCandidates||[]).some(c=>sameMobile(c.mobile,acc.mobile))||((o.status==='open'||o.status==='chat')&&!expired(o))).map(o=>viewFor(o,acc.mobile))});}
      ...
      if(['message','delivery-confirm-request','delivery-confirm-response','delivery-chat-later','delivery-decline','payment-request','payment-response','payment-counter','payment-counter-response','payment-continue','payment-close','payment-update','help-request'].includes(body.action)){
        ...
        let candidate=requestedCandidate(o);
        if(body.action==='delivery-confirm-response')candidate=sessionFor(o,body.peerMobile)||sessionFor(o,o.candidateSessions?.find(s=>s.deliveryConfirmRequest?.status==='pending')?.mobile);
        if(body.action==='payment-response'||body.action==='payment-counter'||body.action==='payment-update')candidate=requestedCandidate(o);
        if(!candidate)return sendJSON(res,403,{ok:false,message:'इस delivery user के लिए chat उपलब्ध नहीं है'});
        ...
        if(body.action==='message'){
          ...
          o.updatedAt=new Date().toISOString();writeOrders();const preview=text?text.replace(/\s+/g,' ').slice(0,120):'📷 Photo भेजी है',sentMessage=candidate.messages[candidate.messages.length-1],recipientMobile=owner?candidate.mobile:o.ownerMobile,recipientName=owner?(candidate.name||'Delivery boy'):(o.ownerName||'Customer'),offlineText=owner?'Customer आपको message कर रहा है। Site में जाकर बात करें।':'Delivery boy आपको message कर रहा है। Site में जाकर बात करें।';addNotification({title:'💬 '+(acc.name||'Customer')+' का message',body:(acc.name||'Customer')+': '+preview,mobile:recipientMobile,kind:'local-delivery-message',orderId:o.id,senderMobile:acc.mobile,messageId:sentMessage.id,actualMessage:true});const offlineAlert=queueOfflineWhatsapp(recipientMobile,recipientName,'chat-message',offlineText,'chat-message:'+sentMessage.id);return sendJSON(res,200,{ok:true,message:sentMessage,offlineAlert});
        }
        if(body.action==='delivery-confirm-request'){
          ...
          if(['pending','rejected','counter-pending','approved'].includes(candidate.paymentRequest?.status))return sendJSON(res,409,{ok:false,message:'पहले delivery payment proposal पूरा करें या chat में तय रकम पर सहमति लें'});
          if(candidate.deliveryConfirmRequest?.status==='pending')return sendJSON(res,409,{ok:false,message:'Customer का जवाब बाकी है'});
          const createdAt = new Date().toISOString();
          candidate.deliveryConfirmRequest={id:'confirm-'+Date.now(),status:'pending',byMobile:acc.mobile,createdAt,expiresAt:new Date(Date.now()+DELIVERY_CONFIRM_TIMEOUT_MS).toISOString()};note('✅ '+(candidate.name||acc.name||'Delivery user')+' ₹'+Number(o.fee||0)+' में delivery करने के लिए तैयार हैं। आपके confirmation का इंतजार है।');writeOrders();notify(o.ownerMobile,'📦 Order pickup request',(candidate.name||acc.name||'Delivery user')+' ने आपका order delivery के लिए लेने की request भेजी है।','local-delivery-confirm-request');const offlineAlert=queueOfflineWhatsapp(o.ownerMobile,o.ownerName,'delivery-request','आपके सामान के लिए delivery request आई है। Site में जाकर accept करें।','delivery-request:'+o.id+':'+acc.mobile);return sendJSON(res,200,{ok:true,order:viewFor(o,acc.mobile),offlineAlert});
        }
        if(body.action==='delivery-confirm-expire'){
          if(owner||o.status!=='chat'||candidate.deliveryConfirmRequest?.status!=='pending')return sendJSON(res,409,{ok:false,message:'Confirmation request अब pending नहीं है'});
          if(Number(new Date(candidate.deliveryConfirmRequest.expiresAt||0))>Date.now())return sendJSON(res,409,{ok:false,message:'Customer confirmation का समय अभी बाकी है'});
          candidate.deliveryConfirmRequest.status='waiting';candidate.deliveryConfirmRequest.expiredAt=new Date().toISOString();note('⏳ Customer confirmation का 5 मिनट समय पूरा हुआ। Chat में बात करके आगे बढ़ें।');writeOrders();return sendJSON(res,200,{ok:true,order:viewFor(o,acc.mobile)});
        }
        if(body.action==='delivery-confirm-response'){
          ...
          if(candidate.deliveryConfirmRequest.expiresAt&&new Date(candidate.deliveryConfirmRequest.expiresAt).getTime()<=Date.now()){candidate.deliveryConfirmRequest.status='waiting';candidate.deliveryConfirmRequest.expiredAt=new Date().toISOString();note('⏳ Customer confirmation का 5 मिनट समय पूरा हुआ। Chat में बात करके आगे बढ़ें।');writeOrders();return sendJSON(res,410,{ok:false,message:'5 मिनट का confirmation समय पूरा हो गया है। Chat में बात करें।'});}
          ...
          if(answer==='yes'){let locks=[];try{locks=JSON.parse(fs.readFileSync(LOCAL_DELIVERY_LOCKS_FILE,'utf8'))||[]}catch(_){}const target=o.alreadyPurchased?Math.round(Number(o.fee||0)):Math.round(Number(o.items||0)+Number(o.fee||0)),requiresWalletLock=target>500&&o.paymentMode!=='cash-on-delivery',row=locks.find(x=>x.orderId===String(o.id)&&sameMobile(x.mobile,o.ownerMobile)&&x.status==='locked');if(target<1)return sendJSON(res,400,{ok:false,message:'Final delivery payment सही नहीं है'});if(requiresWalletLock&&!row&&!walletTxn(acc,'debit',target,{reason:'Local Delivery final booking lock · '+o.id,source:'local_delivery_final_lock',orderId:o.id,operationKey:'local-final-lock:'+o.id+':'+target}))return sendJSON(res,400,{ok:false,message:'Wallet में final booking के लिए पर्याप्त पैसे नहीं हैं'});if(requiresWalletLock&&!row){locks.unshift({orderId:String(o.id),mobile:o.ownerMobile,amount:target,createdAt:new Date().toISOString(),status:'locked'});fs.writeFileSync(LOCAL_DELIVERY_LOCKS_FILE,JSON.stringify(locks.slice(0,1000),null,2));saveAccounts(accounts)}o.customerHold=o.paymentMode==='cash-on-delivery'?0:target;o.status='booked';o.providerMobile=candidate.mobile;o.providerName=candidate.name;o.deliveryCandidates=[{mobile:candidate.mobile,name:candidate.name,acceptedAt:candidate.acceptedAt}];o.candidateSessions=[candidate];o.messages=candidate.messages;o.paymentRequest=candidate.paymentRequest||null;o.deliveryConfirmRequest=candidate.deliveryConfirmRequest;}
          note(answer==='yes'?'✅ Customer ने delivery final confirm कर दी।':answer==='wait'?'⏳ Customer ने Wait चुना है। Chat खुली रहेगी।':'❌ Customer ने अभी delivery final confirm नहीं की। Chat जारी रख सकते हैं।');writeOrders();notify(candidate.mobile,answer==='yes'?'✅ Delivery confirmed':answer==='wait'?'⏳ Delivery confirmation waiting':'❌ Delivery confirmation declined',answer==='yes'?'Customer ने आपको final delivery user चुन लिया है।':answer==='wait'?'Customer ने Wait चुना है, chat जारी रखें।':'Customer ने अभी confirmation नहीं दी।','local-delivery-confirm-response');const offlineAlert=answer==='yes'?queueOfflineWhatsapp(candidate.mobile,candidate.name,'delivery-confirmed','आपका delivery pickup मिल गया है। Site में जाकर update करें।','delivery-confirmed:'+o.id+':'+candidate.mobile):null;return sendJSON(res,200,{ok:true,order:viewFor(o,acc.mobile),offlineAlert});
        }
        ...
        if(body.action==='delivery-decline'){if(owner||o.status!=='chat')return sendJSON(res,403,{ok:false,message:'Delivery छोड़ने का विकल्प उपलब्ध नहीं है'});o.deliveryCandidates=(o.deliveryCandidates||[]).filter(c=>!sameMobile(c.mobile,acc.mobile));o.candidateSessions=(o.candidateSessions||[]).filter(s=>!sameMobile(s.mobile,acc.mobile));if(!o.deliveryCandidates.length){o.status='open';o.providerMobile=null;o.providerName=null}else {const first=o.deliveryCandidates[0];o.providerMobile=first.mobile;o.providerName=first.name}writeOrders();notify(o.ownerMobile,'↩️ Delivery boy ने मना किया','Post अभी अन्य delivery users के लिए available है।','local-delivery-confirm-response');return sendJSON(res,200,{ok:true,order:viewFor(o,acc.mobile)})}
        if(body.action==='payment-request'){
          const fee=Math.round(Number(body.fee||o.fee));if(owner||o.status!=='chat'||!Number.isSafeInteger(fee)||fee<1||fee>100000)return sendJSON(res,400,{ok:false,message:'Final booking के बाद delivery payment नहीं बदली जा सकती'});if(candidate.paymentRequest?.status==='pending')return sendJSON(res,409,{ok:false,message:'Customer का जवाब बाकी है'});candidate.paymentRequest={id:'pay-'+Date.now(),status:'pending',byMobile:acc.mobile,requestedFee:fee,originalFee:Number(o.fee||0),createdAt:new Date().toISOString()};note('💳 Delivery boy ने ₹'+fee+' delivery payment का प्रस्ताव भेजा है। Accept, Reject या Chat में बात करें।');writeOrders();notify(o.ownerMobile,'💳 Payment change request',(candidate.name||acc.name)+' ने नई delivery payment ₹'+fee+' का proposal भेजा है।','local-delivery-payment-request');return sendJSON(res,200,{ok:true,order:viewFor(o,acc.mobile)})
        }
        if(body.action==='payment-response'){
          const answer=String(body.answer||'');if(!owner||candidate.paymentRequest?.status!=='pending'||!['yes','no'].includes(answer))return sendJSON(res,403,{ok:false,message:'यह request उपलब्ध नहीं है'});candidate.paymentRequest.status=answer==='yes'?'approved':'rejected';candidate.paymentRequest.respondedAt=new Date().toISOString();note(answer==='yes'?'✅ Customer ने Yes किया। अब extra delivery payment रकम भरें।':'❌ Customer ने No किया। Helper, क्या आप पुरानी रकम में काम करना चाहते हैं?');writeOrders();notify(candidate.mobile,answer==='yes'?'✅ Payment request approved':'❌ Payment request declined',answer==='yes'?'Customer नई रकम भर रहे हैं।':'Customer ने नई रकम मंज़ूर नहीं की।','local-delivery-payment-response');return sendJSON(res,200,{ok:true,order:viewFor(o,acc.mobile)})
        }
        if(body.action==='payment-counter'){
          const fee=Math.round(Number(body.fee));if(!owner||candidate.paymentRequest?.status!=='rejected'||!Number.isSafeInteger(fee)||fee<1||fee>100000)return sendJSON(res,400,{ok:false,message:'सही counter payment भरें'});candidate.paymentRequest={...candidate.paymentRequest,status:'counter-pending',counterFee:fee,counteredAt:new Date().toISOString()};note('💬 Customer ने ₹'+fee+' का counter offer भेजा है। Delivery boy Yes, No या Chat चुनें।');writeOrders();notify(candidate.mobile,'💬 Customer counter offer','Customer ₹'+fee+' में delivery चाहता है।','local-delivery-payment-response');return sendJSON(res,200,{ok:true,order:viewFor(o,acc.mobile)})
        }
        if(body.action==='payment-counter-response'){
          const answer=String(body.answer||'');if(owner||candidate.paymentRequest?.status!=='counter-pending'||!['yes','no'].includes(answer))return sendJSON(res,400,{ok:false,message:'Counter offer उपलब्ध नहीं है'});candidate.paymentRequest.status=answer==='yes'?'approved':'counter-rejected';if(answer==='yes')candidate.paymentRequest.requestedFee=candidate.paymentRequest.counterFee;note(answer==='yes'?'✅ Delivery boy ने ₹'+candidate.paymentRequest.requestedFee+' का counter offer स्वीकार किया। Customer final payment लागू करें।':'❌ Delivery boy ने counter offer स्वीकार नहीं किया। Chat में बात करें।');writeOrders();notify(o.ownerMobile,answer==='yes'?'✅ Counter offer accepted':'❌ Counter offer declined',answer==='yes'?'Delivery boy आपकी रकम पर तैयार है।':'Delivery boy ने counter offer मना किया।','local-delivery-payment-response');return sendJSON(res,200,{ok:true,order:viewFor(o,acc.mobile)})
        }
      ...
      if(body.action==='message'){
        ...
        if(!['chat','booked','delivering'].includes(o.status))return sendJSON(res,409,{ok:false,message:'इस order में chat available नहीं है'});
        o.messages=Array.isArray(o.messages)?o.messages:[];
        o.messages.push({ id:'m-'+Date.now()+'-'+Math.random().toString(36).slice(2,5),senderMobile:acc.mobile, senderName:acc.name||'Customer', text, photo, recipientMobile:String(String(o.ownerMobile)===String(acc.mobile)?String(body.recipientMobile||o.providerMobile||''):String(o.ownerMobile||'')), time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}), at:new Date().toISOString(), deliveredAt:new Date().toISOString() });
        ...
        const recipient=String(o.ownerMobile)===senderMobile?String(body.recipientMobile||o.providerMobile||''):String(o.ownerMobile||'');
      ...
      if(body.action==='delivery-confirm-request'){
        const o=orders.find(x=>String(x.id)===String(body.orderId||''));
        if(!o||(!(o.deliveryCandidates||[]).some(c=>String(c.mobile)===String(acc.mobile))&&String(o.providerMobile)!==String(acc.mobile))||o.status!=='chat')return sendJSON(res,403,{ok:false,message:'Delivery confirmation उपलब्ध नहीं है'});
        if(o.deliveryConfirmRequest?.status==='pending')return sendJSON(res,409,{ok:false,message:'Customer का जवाब बाकी है'});
        o.deliveryConfirmRequest={id:'confirm-'+Date.now(),status:'pending',byMobile:acc.mobile,createdAt:new Date().toISOString(),expiresAt:new Date(Date.now()+DELIVERY_CONFIRM_TIMEOUT_MS).toISOString()};o.messages=Array.isArray(o.messages)?o.messages:[];o.messages.push({sender:'system',text:'⏳ '+(o.ownerName||'Customer')+' के confirmation का इंतजार है। Delivery boy delivery के लिए ready है।',time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})});
      ...
      if(body.action==='delivery-confirm-response'){
        ...
        if(!o||String(o.ownerMobile)!==String(acc.mobile)||o.deliveryConfirmRequest?.status!=='pending'||!['yes','no','wait'].includes(answer))return sendJSON(res,403,{ok:false,message:'यह confirmation उपलब्ध नहीं है'});
        if(o.deliveryConfirmRequest.expiresAt&&new Date(o.deliveryConfirmRequest.expiresAt).getTime()<=Date.now()){o.deliveryConfirmRequest.status='waiting';o.deliveryConfirmRequest.expiredAt=new Date().toISOString();fs.writeFileSync(LOCAL_DELIVERY_ORDERS_FILE,JSON.stringify(orders.slice(0,5000),null,2));return sendJSON(res,410,{ok:false,message:'5 मिनट का confirmation समय पूरा हो गया है। Chat में बात करें।'});}
        o.deliveryConfirmRequest.status=answer==='yes'?'confirmed':answer==='wait'?'waiting':'rejected';o.deliveryConfirmRequest.respondedAt=new Date().toISOString();if(answer==='yes'){o.status='booked';o.providerMobile=o.deliveryConfirmRequest.byMobile;const chosen=(o.deliveryCandidates||[]).find(c=>String(c.mobile)===String(o.providerMobile));o.providerName=chosen?.name||o.providerName;o.deliveryCandidates=[chosen||{mobile:o.providerMobile,name:o.providerName}]}o.messages=Array.isArray(o.messages)?o.messages:[];o.messages.push({sender:'system',text:answer==='yes'?'✅ Customer ने delivery final confirm कर दी। Order अब केवल customer और selected delivery helper को दिखेगा।':answer==='wait'?'⏳ Customer ने Wait चुना है। Chat खुली रहेगी।':'❌ Customer ने अभी delivery final confirm नहीं की। Chat जारी रख सकते हैं।',time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})});
        fs.writeFileSync(LOCAL_DELIVERY_ORDERS_FILE,JSON.stringify(orders.slice(0,5000),null,2));addNotification({title:answer==='yes'?'✅ Delivery confirmed':answer==='wait'?'⏳ Delivery confirmation waiting':'❌ Delivery confirmation declined',body:answer==='yes'?'Customer ने आपको final delivery user चुन लिया है।':answer==='wait'?'Customer ने Wait चुना है, chat जारी रखें।':'Customer ने अभी confirmation नहीं दी।',mobile:o.providerMobile,kind:'local-delivery-confirm-response',orderId:o.id});return sendJSON(res,200,{ok:true,order:o});
      }
      if(body.action==='delivery-chat-later'){
        const o=orders.find(x=>String(x.id)===String(body.orderId||''));
        if(!o||String(o.providerMobile)!==String(acc.mobile)||o.status!=='chat')return sendJSON(res,403,{ok:false,message:'यह विकल्प उपलब्ध नहीं है'});
        o.deliveryConfirmRequest={status:'chat',at:new Date().toISOString()};o.messages=Array.isArray(o.messages)?o.messages:[];o.messages.push({sender:'system',text:'💬 Delivery boy ने कहा है कि delivery final करने से पहले chat में बात करेंगे।',time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})});
        fs.writeFileSync(LOCAL_DELIVERY_ORDERS_FILE,JSON.stringify(orders.slice(0,5000),null,2));return sendJSON(res,200,{ok:true,order:o});
      }
      if(body.action==='delivery-decline'){
        const o=orders.find(x=>String(x.id)===String(body.orderId||''));
        if(!o||String(o.providerMobile)!==String(acc.mobile)||o.status!=='chat')return sendJSON(res,403,{ok:false,message:'Delivery छोड़ने का विकल्प उपलब्ध नहीं है'});
        o.status='open';o.providerMobile=null;o.providerName=null;o.deliveryConfirmRequest={status:'declined-by-helper',at:new Date().toISOString()};o.messages=Array.isArray(o.messages)?o.messages:[];o.messages.push({sender:'system',text:'Delivery boy ने delivery के लिए No चुना। Post फिर से home पर available है।',time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})});
      ...
      if(body.action==='payment-update'){
        ...
        if(o.paymentMode!=='cash-on-delivery'&&change>0&&!walletTxn(acc,'debit',change,{reason:'Local Delivery payment increase · '+o.id,source:'local_delivery_lock_adjust',orderId:o.id,operationKey:'local-payment-adjust:'+o.id+':'+target}))return sendJSON(res,400,{ok:false,message:'Wallet में नई रकम के लिए पर्याप्त पैसे नहीं हैं'});
        ...
        o.items=items;o.fee=fee;o.customerHold=o.paymentMode==='cash-on-delivery'?0:target;o.paymentRequest={...o.paymentRequest,status:'applied',appliedAt:new Date().toISOString()};if(o.status==='chat'&&o.providerMobile)o.status='booked';o.messages=Array.isArray(o.messages)?o.messages:[];o.messages.push({sender:'system',text:'💰 नई delivery payment ₹'+fee+' लागू हुई। Wallet lock अब ₹'+target+' है।',time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})});
      ...
      if(body.action==='actual-bill-response'){
        ...
        let locks=[];try{locks=JSON.parse(fs.readFileSync(LOCAL_DELIVERY_LOCKS_FILE,'utf8'))||[]}catch(_){}const row=locks.find(x=>x.orderId===String(o.id)&&String(x.mobile)===String(acc.mobile)&&x.status==='locked'),target=o.alreadyPurchased?Math.round(Number(o.fee||0)):Math.round(Number(o.actualBillRequest.amount||0)+Number(o.fee||0)),walletRequired=target>500&&o.paymentMode!=='cash-on-delivery',current=Number(row?.amount||0),change=target-current;
        if(walletRequired&&change>0&&!walletTxn(acc,'debit',change,{reason:'Actual bill lock increase · '+o.id,source:'local_delivery_actual_bill',orderId:o.id,operationKey:'local-bill-lock:'+o.id+':'+target+':'+(change>0?'debit':'credit')}))return sendJSON(res,400,{ok:false,message:'Wallet में actual bill के लिए पर्याप्त पैसे नहीं हैं'});if(walletRequired&&change<0)walletTxn(acc,'credit',Math.abs(change),{reason:'Actual bill lock refund · '+o.id,source:'local_delivery_actual_bill',orderId:o.id,operationKey:'local-bill-lock:'+o.id+':'+target+':'+(change>0?'debit':'credit')});if(!walletRequired&&row&&current>0){walletTxn(acc,'credit',current,{reason:'₹500 तक Local Delivery free-limit unlock · '+o.id,source:'local_delivery_free_limit_unlock',orderId:o.id,operationKey:'local-free-unlock:'+o.id});row.status='released';row.releasedAt=new Date().toISOString()}else if(walletRequired&&row){row.amount=target;row.updatedAt=new Date().toISOString()}else if(walletRequired&&target>0)locks.unshift({orderId:String(o.id),mobile:acc.mobile,amount:target,createdAt:new Date().toISOString(),status:'locked'});o.items=Number(o.actualBillRequest.amount);o.customerHold=o.paymentMode==='cash-on-delivery'?0:target;o.actualBillRequest.status='approved';o.actualBillRequest.respondedAt=new Date().toISOString();o.messages=Array.isArray(o.messages)?o.messages:[];o.messages.push({sender:'system',text:'✅ Actual bill ₹'+o.items+' approved। '+(walletRequired?'Wallet lock अब ₹'+target+' है।':'₹500 तक होने से wallet lock नहीं लगा।'),time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})});fs.writeFileSync(LOCAL_DELIVERY_LOCKS_FILE,JSON.stringify(locks.slice(0,1000),null,2));fs.writeFileSync(LOCAL_DELIVERY_ORDERS_FILE,JSON.stringify(orders.slice(0,5000),null,2));saveAccounts(accounts);addNotification({title:'✅ Actual bill approved',body:walletRequired?'Customer ने actual bill approve किया और wallet lock update हो गया।':'Customer ने actual bill approve किया। ₹500 तक होने से wallet lock नहीं लगा।',mobile:o.providerMobile,kind:'local-delivery-bill-response',orderId:o.id});return sendJSON(res,200,{ok:true,order:o,locked:walletRequired?target:0,walletBalance:acc.walletBalance});
      }
      if(body.action==='delivery-otp-create'){
        ...
        let locks=[];try{locks=JSON.parse(fs.readFileSync(LOCAL_DELIVERY_LOCKS_FILE,'utf8'))||[]}catch(_){}
        const locked=locks.find(x=>x.orderId===String(o.id)&&String(x.mobile)===String(o.ownerMobile)&&x.status==='locked'),otpTotal=o.alreadyPurchased?Math.round(Number(o.fee||0)):Math.round(Number(o.items||0)+Number(o.fee||0));
        if(o.paymentMode!=='cash-on-delivery'&&otpTotal>500&&(!locked||!Number.isSafeInteger(Number(locked.amount))||Number(locked.amount)<1))return sendJSON(res,409,{ok:false,message:'Wallet lock सुरक्षित नहीं है। Help से संपर्क करें।'});
        const now=Date.now(),existing=o.deliveryOtp,expired=existing&&Number(existing.expiresAt||0)<=now;
        if(existing?.code&&!expired&&!existing.lockedAt)return sendJSON(res,200,{ok:true,code:existing.code,expiresAt:existing.expiresAt,attemptsRemaining:Math.max(0,5-Number(existing.attempts||0))});
        const code=String(Math.floor(100000+Math.random()*900000));o.deliveryOtp={code,createdAt:now,expiresAt:now+30*60*1000,attempts:0,maxAttempts:5};
        fs.writeFileSync(LOCAL_DELIVERY_ORDERS_FILE,JSON.stringify(orders.slice(0,5000),null,2));addNotification({title:'✅ सामान received',body:'Customer ने नया delivery OTP बनाया है। यह 30 मिनट तक valid है।',mobile:o.providerMobile,kind:'local-delivery-otp',orderId:o.id});return sendJSON(res,200,{ok:true,code,expiresAt:o.deliveryOtp.expiresAt,attemptsRemaining:5});
      }
      if(body.action==='delivery-otp-verify'){
        const o=orders.find(x=>String(x.id)===String(body.orderId||'')),code=String(body.code||'');
        if(!o||String(o.providerMobile)!==String(acc.mobile)||o.status!=='delivering'||!o.deliveryOtp)return sendJSON(res,400,{ok:false,message:'OTP उपलब्ध नहीं है'});
        const otp=o.deliveryOtp,now=Date.now();
        if(Number(otp.expiresAt||0)<=now){otp.expiredAt=now;fs.writeFileSync(LOCAL_DELIVERY_ORDERS_FILE,JSON.stringify(orders.slice(0,5000),null,2));return sendJSON(res,410,{ok:false,message:'OTP expire हो गया। Customer से नया OTP लें।'});}
        if(otp.lockedAt)return sendJSON(res,429,{ok:false,message:'बहुत गलत OTP attempts हुए हैं। Customer से नया OTP लें।'});
        ...
        let locks=[];try{locks=JSON.parse(fs.readFileSync(LOCAL_DELIVERY_LOCKS_FILE,'utf8'))||[]}catch(_){}const row=locks.find(x=>x.orderId===String(o.id)&&String(x.mobile)===String(o.ownerMobile)&&x.status==='locked'),amount=Number(row?.amount||0),otpTotal=o.alreadyPurchased?Math.round(Number(o.fee||0)):Math.round(Number(o.items||0)+Number(o.fee||0)),walletRequired=otpTotal>500&&o.paymentMode!=='cash-on-delivery';
        if(walletRequired&&(!row||!Number.isSafeInteger(amount)||amount<1))return sendJSON(res,409,{ok:false,message:'Wallet lock सुरक्षित नहीं है। Delivery complete नहीं की जा सकती। Help से संपर्क करें।'});
        if(walletRequired&&!walletTxn(acc,'credit',amount,{reason:'Local Delivery completed · '+o.id,source:'local_delivery_settlement',orderId:o.id,operationKey:'local-settlement:'+o.id+':'+acc.mobile}))return sendJSON(res,500,{ok:false,message:'Payment settlement नहीं हुआ। दुबारा OTP submit न करें; Help से संपर्क करें।'});
        if(walletRequired&&row){row.status='settled';row.settledAt=new Date().toISOString()}o.status='completed';o.completedAt=new Date().toISOString();if(o.paymentMode==='cash-on-delivery')o.paymentStatus='cash-collected';o.deliveryOtp.verifiedAt=o.completedAt;o.messages=Array.isArray(o.messages)?o.messages:[];o.messages.push({sender:'system',text:walletRequired?'🎉 Delivery successfully completed! OTP verify हो गया और payment ₹'+amount+' delivery boy के wallet में भेज दिया गया।':o.paymentMode==='cash-on-delivery'?'🎉 Delivery successfully completed! OTP verify हो गया। Cash on Delivery की रकम customer से सीधे ली गई; wallet में कोई credit नहीं हुआ।':'🎉 Delivery successfully completed! OTP verify हो गया। ₹500 तक होने से wallet lock और settlement की जरूरत नहीं थी।',time:new Date().toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'})});fs.writeFileSync(LOCAL_DELIVERY_LOCKS_FILE,JSON.stringify(locks,null,2));fs.writeFileSync(LOCAL_DELIVERY_ORDERS_FILE,JSON.stringify(orders.slice(0,5000),null,2));saveAccounts(accounts);addNotification({title:'🎉 Delivery successfully completed',body:'OTP verify हो गया। आपका order सफलतापूर्वक पूरा हुआ।',mobile:o.ownerMobile,kind:'local-delivery-completed',orderId:o.id});return sendJSON(res,200,{ok:true,walletBalance:acc.walletBalance,amount:walletRequired?amount:0,order:o});
      }
      ...
      if(body.action==='customer-rating'){
        const o=orders.find(x=>String(x.id)===String(body.orderId||'')),score=Math.round(Number(body.score)),note=String(body.note||'').trim().slice(0,300);
        if(!o||String(o.ownerMobile)!==String(acc.mobile)||o.status!=='completed'||!Number.isSafeInteger(score)||score<1||score>5)return sendJSON(res,400,{ok:false,message:'Rating उपलब्ध नहीं है'});
        if(o.customerRating)return sendJSON(res,409,{ok:false,message:'Rating पहले ही दी जा चुकी है'});
        o.customerRating={score,note,byMobile:acc.mobile,at:new Date().toISOString()};fs.writeFileSync(LOCAL_DELIVERY_ORDERS_FILE,JSON.stringify(orders.slice(0,5000),null,2));notify(o.providerMobile,'⭐ New delivery rating',(acc.name||'Customer')+' ने आपको '+score+'/5 rating दी है।','local-delivery-rating');return sendJSON(res,200,{ok:true,rating:o.customerRating});
      }
  ...
  if (req.method === 'GET' && urlPath === '/admin/live-json') {
    const accounts = loadAccounts();
    const rawPendingOtps = loadOtpRequests().filter(r => !r.verified);
    const pendingResets = accounts.filter(a => a.pinResetRequested).map(a => {
      const row = rawPendingOtps.find(r => r.purpose === 'pin_reset' && String(r.mobile) === String(a.mobile));
      return { id: a.id, name: a.name, mobile: a.mobile, at: a.pinResetRequestedAt || null,
        otp: row && row.manualOtp || '', expiresAt: row && row.expiresAt || null };
    });
    const pendingOtps = rawPendingOtps.map(r => ({
      mobile: r.mobile, name: r.name || '', id: r.id || '',
      at: r.createdAt || null, expiresAt: r.expiresAt || null,
      manualOtp: r.manualOtp || '', delivery: r.delivery || 'sms', purpose: r.purpose || ''
    }));
    const codes = loadCodes();
...
loadOrdersPage();setInterval(loadOrdersPage,20000);
  ...
  if (req.method === 'GET' && urlPath === '/admin') {
    ...
    const otpCards = pendingOtps.filter(r => r.purpose !== 'pin_reset').map(r => {
...
${(function(){
  ...
  return list.map((p,i)=>{
    ...
    return '<div style="width:220px;border:1px solid rgba(212,175,55,.4);border-radius:9px;padding:7px;background:#15120b;position:relative">'
      ...
      + '<label class="muted" style="display:block;font-size:10px">Up / Down <b id="heroBgYVal'+i+'">'+y+'%</b><input type="range" id="heroBgY'+i+'" min="0" max="100" value="'+y+'" oninput="adminPreviewHeroBgCrop('+i+')"></label>'
      + '<label class="muted" style="display:block;font-size:10px">Zoom <b id="heroBgZVal'+i+'">'+Math.round(z*100)+'%</b><input type="range" id="heroBgZ'+i+'" min="100" max="250" value="'+Math.round(z*100)+'" oninput="adminPreviewHeroBgCrop('+i+')"></label>'
...
function renderOtps(list) {
  ...
  if (!box) return;
  list = (list || []).filter(function(r) { return r.purpose !== 'pin_reset'; });
...
function adminPreviewHeroBgCrop(index) {
  ...
  var preview = document.getElementById('heroBgPreview' + index);
  if (preview) {
    preview.style.backgroundPosition = x + '% ' + y + '%';
    preview.style.transform = 'scale(' + zoom + ')';
  }
  var xv = document.getElementById('heroBgXVal' + index), yv = document.getElementById('heroBgYVal' + index), zv = document.getElementById('heroBgZVal' + index);
