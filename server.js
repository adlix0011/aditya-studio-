  ...
  if (req.method === 'POST' && urlPath === '/api/local-delivery/orders') {
    try {
      ...
      if(body.action==='create'){
        ...
        const paidProductLimit=500,paidProductAmount=Math.round(Number(o.items||0));
        if(o.alreadyPurchased&&paidProductAmount>paidProductLimit&&Number(acc.walletBalance||0)<paidProductAmount)return sendJSON(res,400,{ok:false,needsRecharge:true,requiredWallet:paidProductAmount,message:'₹500 से अधिक paid product के लिए पहले wallet में ₹'+paidProductAmount+' add करें।'});
